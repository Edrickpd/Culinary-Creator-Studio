...
app.get("/api/health", (_req: Request, res: Response) => {
  res.json({
    status: "ok",
    environment: process.env.VERCEL ? "vercel-serverless" : "local-express",
...
app.post("/api/stripe/create-checkout-session", async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const origin = returnUrl ? returnUrl.split('#')[0] : 'https://culinary-creator-studio.vercel.app';
